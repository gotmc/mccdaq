	...
	c.Convey("Given the need to convert binary values into voltages", t, func() {
		for _, testCase := range testCases {
			...
			c.Convey(conveyance, func() {
				...
				c.Convey(conveyance, func() {
					computedValue, _ := RawVoltsFromWord(testCase.binaryValue, testCase.voltageRange)
...
func TestConvert5VRangeBinaryValueToVoltage(t *testing.T) {
	testCases := []struct {
		voltage      float32
		voltageRange VoltageRange
		binaryValue  []byte
		...
		{4.999847412, Range5V, []byte{0xff, 0xff}},
	}
	c.Convey("Given the need to convert binary values into voltages", t, func() {
		for _, testCase := range testCases {
			conveyance := fmt.Sprintf("When the binary value is %#x (little endian)", testCase.binaryValue)
